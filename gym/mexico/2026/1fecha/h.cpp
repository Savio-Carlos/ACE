...
void solve() {
    ...
    int mx = 0;
    for(int i = 0; i < n; i++) {
        if(a[i] == b[n-1-i] && b[i] == a[n-1-i]) mx = i + 1;
        else break;
    }

    auto mnc_a = manacher(a);
    auto mnc_b = manacher(b);
    debug(mnc_a);
    debug(mnc_b);
    ...
    // impares
    for(int i = 0; i < n; i++) {
        int ra = (mnc_a[2*i + 1] - 2) / 2;
        int rb = (mnc_b[2*i + 1] - 2) / 2;
        int raio = min(ra, rb);

        debug(ra,rb,raio,i);
        int l = max<int>(0, i - mx);
        int r = min(raio, i - 1);
        if(r >= l) cnt1 += r - l + 1;
        debug(l,r);
        
        l = max<int>(0, n - 1 - mx - i);
        r = min(raio, n - 2 - i);
        if(r >= l) cnt2 += r - l + 1;
        debug(l,r);
    ...
    // pares
    for(int i = 0; i < n - 1; i++) {
        int ra = (mnc_a[2*i + 2] - 1) / 2;
        int rb = (mnc_b[2*i + 2] - 1) / 2;
        int raio = min(ra, rb);
        debug(ra, mnc_a[2*i+2]);
        int l = max<int>(1, i - mx + 1);
        int r = min(raio, i);
        if(r >= l) cnt1 += r - l + 1;
        debug(i, l, r);

        l = max<int>(1, n - 1 - mx - i);
        r = min(raio, n - 2 - i);
        debug(l,r);
