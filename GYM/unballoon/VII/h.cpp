...
signed main(){
    ...
    while(q--){
        ...
        cin >> t >> x;
        if (t == 1){
            auto fatores = factor(x);
            for (auto [u, qt] : fatores) {
                cnt[u] = min(cnt[u] + qt, LIMITE);
                if (cnt[u] < LIMITE) cur.insert(u);
            }
        }
        else if (t == 2){
            vector<int> tirar;
            for (auto u : cur) {
                cnt[u] = min(cnt[u] * x, LIMITE);
                if (cnt[u] >= LIMITE) tirar.push_back(u);
            }
            for (auto u : tirar) cur.erase(u);
        }
        else {
            auto fatores = factor(x);
            bool pos = true;
