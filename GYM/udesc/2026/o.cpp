...
namespace Segtree {
    ...
    int query(int a, int b, int node = 1, int l = 0, int r = n-1){
        ...
        int m = (l+r)/2;
        return query(a, b, node*2, l, m) + query(a, b, node*2+1, m+1, r);
    }

    int update(int i, int x, int node = 1, int l = 0, int r = n-1){
        if (l == r) tree[node] += x; 
        int m = (l+r)/2;
        if (i <= m) return query(a, b, node*2, l, m) 
        else return query(a, b, node*2+1, m+1, r);
    }
}
