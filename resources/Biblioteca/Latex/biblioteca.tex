...
\title{\begin{CJK}{UTF8}{gbsn}身正不怕影子斜\end{CJK} \\ Unioeste}
\author{Amós, Carlos e Eduarda}
\phrase{Um pé reto não tem medo de um sapato torto.}

% Mapeia acentos UTF-8 para o listings nao quebrar em bytes multibyte
\lstset{literate=
	{~}{{$\sim$}}1
	...
	{ú}{{\'u}}1 {ü}{{\"u}}1
	{ç}{{\c c}}1
	{Á}{{\'A}}1 {Í}{{\'I}}1 {É}{{\'E}}1 {Ó}{{\'O}}1 {Ç}{{\c C}}1
}
...
\subsection{Knapsack II}
\begin{lstlisting}
7d2 int n, W;
dfd int arr[MAXN], weights[MAXN];

d09 int pd(int i, int val){
966     if(val < 0) return INF;
2e2     if(val == 0) return 0;
847     if(i == 0) return INF;
    
e8e     if(dp[i][val] != -1) return dp[i][val];
    
31b     return dp[i][val] = min(pd(i-1, val - arr[i]) + weights[i], pd(i-1, val));
522 }
\end{lstlisting}

\subsection{Knapsack recursivo}
\begin{lstlisting}

dca int n, k;
ede int pd(int w, int i){
7a7     if(w > k) return -INF;
12b     if(i == n) return 0;
260     if(dp[i][w] != -1) return dp[i][w];
    
5f3     return dp[i][w] = max(pd(w+peso[i], i+1) + val[i], pd(w, i+1));
f97 }
\end{lstlisting}
...
\begin{lstlisting}
492 for (auto x : a){
62d     int pos = upper_bound(all(lis), x) - lis.begin();
621     if (pos == (int)lis.size()) lis.push_back(x);
846     else lis[pos] = x;
498 }
\end{lstlisting}
...
3eb using Tree =
2d0     tree<T, null_type, less<T>, rb_tree_tag, tree_order_statistics_node_update>;
...
\subsection{SegTree Ultimate (Lazy Propagation, generica)}
\begin{lstlisting}
...
e7d template<typename Node, typename Tag>
383 struct SegTree {
1a8     int n;
8a5     vector<Node> tree;
067     vector<Tag>  lazy;
    
0a9     SegTree() : n(0) {}
c7f     SegTree(int n) { assign(n); }
65f     template<typename U> SegTree(const vector<U>& v) { build(v); }
    
efc     void assign(int n_) {                    // n folhas neutras (Node())
e95         n = n_;
f85         tree.assign(4*n, Node());
8b5         lazy.assign(4*n, Tag());
45d     }
e52     template<typename U> void build(const vector<U>& v) {
d6c         n = v.size();
f85         tree.assign(4*n, Node());
8b5         lazy.assign(4*n, Tag());
4c2         if (n) build(v, 1, 0, n-1);
abf     }
    
1e1     void applyTo(int node, const Tag& t) {   // aplica t no node e acumula no lazy
23c         tree[node].apply(t);
a45         lazy[node].apply(t);
aa4     }
434     void push(int node) {                    // empurra lazy pros filhos
fee         if (!lazy[node].empty()) {
e7e             applyTo(2*node, lazy[node]);
bc9             applyTo(2*node+1, lazy[node]);
e71             lazy[node] = Tag();
04c         }
0b5     }
    
477     template<typename U>
a54     void build(const vector<U>& v, int node, int l, int r) {
94f         if (l == r) { tree[node] = Node(v[l]); return; }
ee4         int m = (l+r)/2;
f95         build(v, 2*node, l, m);
c67         build(v, 2*node+1, m+1, r);
8fb         tree[node] = tree[2*node] + tree[2*node+1];
a09     }
    
dc5     void update(int ql, int qr, const Tag& t) { if (n) update(1,0,n-1,ql,qr,t); }
112     void update(int node, int l, int r, int ql, int qr, const Tag& t) {
8a6         if (qr < l || r < ql) return;
665         if (ql <= l && r <= qr) { applyTo(node, t); return; }
b55         push(node);
ee4         int m = (l+r)/2;
4b1         update(2*node, l, m, ql, qr, t);
c03         update(2*node+1, m+1, r, ql, qr, t);
8fb         tree[node] = tree[2*node] + tree[2*node+1];
578     }
    
318     Node query(int ql, int qr) { return n ? query(1,0,n-1,ql,qr) : Node(); }
b1e     Node query(int node, int l, int r, int ql, int qr) {
9d4         if (qr < l || r < ql) return Node();
ea7         if (ql <= l && r <= qr) return tree[node];
b55         push(node);
ee4         int m = (l+r)/2;
a6e         return query(2*node, l, m, ql, qr) + query(2*node+1, m+1, r, ql, qr);
c59     }
    
3e6     void pointSet(int i, const Node& val) { pointSet(1,0,n-1,i,val); }
460     void pointSet(int node, int l, int r, int i, const Node& val) {
c47         if (l == r) { tree[node] = val; return; }
b55         push(node);
ee4         int m = (l+r)/2;
28e         if (i <= m) pointSet(2*node, l, m, i, val);
a37         else        pointSet(2*node+1, m+1, r, i, val);
8fb         tree[node] = tree[2*node] + tree[2*node+1];
0a2     }
...
1e0     template<typename F> int find_first(int ql, int qr, const F& pred) {
db5         Node acc = Node();
82f         return find_first(1, 0, n-1, ql, qr, pred, acc);
efc     }
398     template<typename F>
9ec     int find_first(int node, int l, int r, int ql, int qr, const F& pred, Node& acc) {
96e         if (qr < l || r < ql) return -1;
ce3         if (ql <= l && r <= qr) {
609             Node cand = acc + tree[node];
06b             if (!pred(cand)) { acc = cand; return -1; }
40c             while (l < r) {                  // achou no bloco: desce pra folha
b55                 push(node);
ee4                 int m = (l+r)/2;
c6a                 Node left = acc + tree[2*node];
a68                 if (pred(left)) { node = 2*node;   r = m; }
119                 else            { acc = left; node = 2*node+1; l = m+1; }
7b6             }
792             return l;
2ef         }
b55         push(node);
ee4         int m = (l+r)/2;
4c1         int res = find_first(2*node, l, m, ql, qr, pred, acc);
3b7         if (res != -1) return res;
1dc         return find_first(2*node+1, m+1, r, ql, qr, pred, acc);
e7c     }
3e1 };
...
// =============================================================================
622 const ll MOD = 1e9 + 7;
e54 struct mint {
aa3     ll v;
639     mint(ll x = 0) { v = x % MOD; if (v < 0) v += MOD; }
dfa     mint& operator+=(const mint& o){ v += o.v; if (v >= MOD) v -= MOD; return *this; }
d7c     mint& operator-=(const mint& o){ v -= o.v; if (v < 0)   v += MOD; return *this; }
23e     mint& operator*=(const mint& o){ v = v * o.v % MOD; return *this; }
bca     mint pow(ll e) const { mint r = 1, b = *this; while (e) { if (e&1) r *= b; b *= b; e >>= 1; } return r; }
345     mint inv() const { return pow(MOD - 2); }
878     mint& operator/=(const mint& o){ return *this *= o.inv(); }
acc     friend mint operator+(mint a, const mint& b){ return a += b; }
091     friend mint operator-(mint a, const mint& b){ return a -= b; }
962     friend mint operator*(mint a, const mint& b){ return a *= b; }
5af     friend mint operator/(mint a, const mint& b){ return a /= b; }
369     friend bool operator==(const mint& a, const mint& b){ return a.v == b.v; }
008     friend bool operator!=(const mint& a, const mint& b){ return a.v != b.v; }
40a     friend ostream& operator<<(ostream& os, const mint& m){ return os << m.v; }
5dd };
...
928 struct Compressor {
cff     vector<T> vals;
ab9     void add(T x) { vals.push_back(x); }
0a8     void build() {
3d9         sort(vals.begin(), vals.end());
7ee         vals.erase(unique(vals.begin(), vals.end()), vals.end());
e43     }
f84     int size() const { return (int)vals.size(); }
6a5     int idx(T x) const { return (int)(lower_bound(vals.begin(), vals.end(), x) - vals.begin()); }
a2c     T   val(int i) const { return vals[i]; }
c13 };
...
7e5 struct Tag {
8bb     ll add = 0;
be7     void apply(const Tag& t){ add += t.add; }
ece     bool empty() const { return add == 0; }
ac9 };
bf2 struct Node {
ab5     ll sum = 0; int len = 0;
2dd     Node() {}
42c     Node(ll x): sum(x), len(1) {}
84a     void apply(const Tag& t){ sum += t.add * len; }
3b7     friend Node operator+(const Node& a, const Node& b){
638         Node r; r.sum = a.sum + b.sum; r.len = a.len + b.len; return r;
494     }
b0e };
...
ac9 };
bf2 struct Node {
e83     ll mx = LLONG_MIN; int len = 0;
2dd     Node() {}
2b8     Node(ll x): mx(x), len(1) {}
c4f     void apply(const Tag& t){ mx += t.add; }
3b7     friend Node operator+(const Node& a, const Node& b){
ad9         Node r; r.mx = max(a.mx, b.mx); r.len = a.len + b.len; return r;
dbb     }
765 };

// ---- (3) MIN de range + ASSIGN em range ----  st.query(l,r).mn
7e5 struct Tag {
f2a     ll val = 0; bool set = false;
afd     void apply(const Tag& t){ if (t.set) { val = t.val; set = true; } }
881     bool empty() const { return !set; }
2eb };
bf2 struct Node {
e4e     ll mn = LLONG_MAX; int len = 0;
2dd     Node() {}
77d     Node(ll x): mn(x), len(1) {}
127     void apply(const Tag& t){ if (t.set) mn = t.val; }
3b7     friend Node operator+(const Node& a, const Node& b){
de4         Node r; r.mn = min(a.mn, b.mn); r.len = a.len + b.len; return r;
624     }
a86 };
...
68f     mint add = 0;
be7     void apply(const Tag& t){ add += t.add; }
4d8     bool empty() const { return add == mint(0); }
c14 };
bf2 struct Node {
a6a     mint sum = 0; int len = 0;
2dd     Node() {}
3f2     Node(mint x): sum(x), len(1) {}
479     void apply(const Tag& t){ sum += t.add * mint(len); }
3b7     friend Node operator+(const Node& a, const Node& b){
638         Node r; r.sum = a.sum + b.sum; r.len = a.len + b.len; return r;
494     }
899 };
...
2dd     Node() {}
42c     Node(ll x): sum(x), len(1) {}
0a9     void apply(const Tag& t){ if (t.set) sum = t.val * len; }
3b7     friend Node operator+(const Node& a, const Node& b){
638         Node r; r.sum = a.sum + b.sum; r.len = a.len + b.len; return r;
494     }
d97 };
...
7f9         bool res; 
bdc         edge(int to_, int cap_, int rev_, bool res_) : to(to_), cap(cap_), rev(rev_), flow(0), res(res_) {}
f70     };
d8f     vector<vector<edge>> graph;
afd     vector<int> level, beg;
1a8     int n;
805     int fluxo;
     
489     Dinic(int n) : n(n), graph(n), level(n), beg(n), fluxo(0) {}
     
087     void add(int a, int b, int c) {
41b         graph[a].emplace_back(b, c, (int)graph[b].size(), false);
679         graph[b].emplace_back(a, 0, (int)graph[a].size() - 1, true);
036     }
     
123     bool bfs(int s, int t) {
224         fill(level.begin(), level.end(), -1);
9ef         fill(beg.begin(), beg.end(), 0);
ec8         level[s] = 0; 
26a         queue<int> q;
08b         q.push(s);
14d         while (!q.empty()) {
e4a             int u = q.front(); 
833             q.pop();
d64             for (auto &e : graph[u]){
588                 if (level[e.to] != -1 or (e.flow == e.cap)) continue;
0ab                 level[e.to] = level[u] + 1;
6f4                 q.push(e.to);
746             }
...
036     int dfs(int v, int t, int f = INF) {
4ff         if (v == t or !f) return f;
     
af4         for (int &i = beg[v]; i < (int)graph[v].size(); i++) {
c12             auto &e = graph[v][i];
2da             if (level[e.to] != level[v] + 1) continue;
     
81f             int d = dfs(e.to, t, min(f, e.cap - e.flow));
                
53a             if (!d) continue; 
60b             e.flow += d;
238             graph[e.to][e.rev].flow -= d;
be2             return d;
a5b             }
bb3         return 0;
d3f     }
     
ff1     int max_flow(int s, int t) {
8ce         while (bfs(s, t)) {
9ee             while (int f = dfs(s, t)) fluxo += f;
df8         }
7f3         return fluxo;
438     }
     
3ef     vector<pair<int, int>> get_cut(int s, int t) {
d93         if (!fluxo) max_flow(s, t);
68c         vector<pair<int, int>> cut;
afd         vector<int> vis(graph.size(), 0), st = {s};
321         vis[s] = 1;
3c6         while (st.size()) {
b17             int u = st.back(); st.pop_back();
665             for (auto e : graph[u]) if (!vis[e.to] and e.flow < e.cap){
c17                 vis[e.to] = 1, st.push_back(e.to);
29b             }
388         }
8ec         for (int i = 0; i < graph.size(); i++) for (auto e : graph[i]){
...
1bb     vector<vector<int>> get_paths(int s, int t){
d93         if(!fluxo) max_flow(s,t);
437         vector<vector<int>> paths;
331         vector<int> ptr(n, 0);
    
9af         for (int k = 0; k < fluxo; k++) {
1ca             vector<int> path = {s};
299             int cur = s;
    
c30             while (cur != t) {
666                 bool adv = false;
a04                 for (int &i = ptr[cur]; i < (int)graph[cur].size(); i++) {
ee0                     auto &e = graph[cur][i];
8fa                     if (e.flow > 0 && !e.res) {
518                         e.flow--;
8f3                         cur = e.to;
...
9fb                 if (!adv) break;
d62             }
8fe             if (cur == t) paths.push_back(path);
411         }
...
803 int det = (c1 * c4) - (c2 * c3);
2e5 int det_x = (n * c4) - (c2 * m);
ec2 int det_y = (c1 * m) - (n * c3);
...
477     template<typename U> 
ff5     pt operator / (const U c) const { return pt(x / c, y / c); }
477     template<typename U> 
...
9dc vector<pt> convex(vector<pt> v){
304     sort(all(v));
64f     v.erase(unique(all(v)), v.end());
        /*
31d     IMPORTANTE! em casos que pede o convex hull incluindo pontos colineares, mudar o ccw para >=
c4c     */
52d     if(v.size() <= 1) return v;
526     vector<pt> l, u;
f14     for(int i = 0; i < v.size(); i++){
e35         while(l.size() > 1 and !ccw(l.end()[-2], l.end()[-1], v[i])){
364             l.pop_back();
b47         }
c35         l.push_back(v[i]);
189     }
3e9     for(int i = v.size() - 1; i >= 0; i--){
0c7         while(u.size() > 1 and !ccw(u.end()[-2], u.end()[-1], v[i])){
7a8             u.pop_back();
9bd         }
a95         u.push_back(v[i]);
625     }
...
\subsection{3 equacoes 4 variaveis}
\begin{lstlisting}
// e(s) = (y+p-r)/2 - s
// w(s) = (y-p-r)/2 - s
// s(s) = s
...
\subsection{Achar prefixos que formam a string toda}
\begin{lstlisting}
...
9e0 template<int MOD> struct str_hash {
c63 	static int P;
648 	vector<int> h, p;
...
1ef 	}
6cf 	int operator()(int l, int r) { // retorna hash s[l...r]
796 		int hash = h[r] - (l ? h[l - 1]*p[r - l + 1]%MOD : 0);
...
63d void solve() {
21a     string s; cin >> s;
556     str_hash<MOD> hasher(s);
163     int n = s.size();
78a     for(int i = 1; i <= n; i++){
bdd         if(i == n) cout << n << endl;
4e6         else{
9f5             int h1 = hasher(0, n-i-1);
576             int h2 = hasher(i, n-1);
18b             if(h1 == h2){
bfe                 cout << i << " ";
ef7             }
d8b         }
...
\subsection{Dynamic Inversions}
\begin{lstlisting}
/*
115 You are given a permutation {1,2,3,. . . ,n}. Remove m of them one by one, and output the number of
...
add     int n, m, k;
5fe     cin >> n >> m >> k;
70d     vector<vector<int>> con(n, vector<int>(n));
dd5     for (int i = 0; i < m; i++){
ba2         int a, b;
0a8         cin >> a >> b;
67f         con[--a][--b]++;
...
add     int n, m, k;
5fe     cin >> n >> m >> k;
067     vector<vector<int>> con(n, vector<int>(n, INF));
dd5     for (int i = 0; i < m; i++){
70f         int a, b, c;
e22         cin >> a >> b >> c;
583         a--;
199         b--;
d81         con[a][b] = min(c, con[a][b]);
946     }
db1     matrix mt(con);
654     mt = mt^k; 
f57     int ans = mt[0][n-1];
4bc     if (ans == INF) cout << "-1" << endl;
519     else cout << ans << endl;
